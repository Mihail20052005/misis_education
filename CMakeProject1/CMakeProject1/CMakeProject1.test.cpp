...
int main()
{
	...
	for (int i = 1; i > 0; ++i) {
		...
		//	std::cout << "Core 1 {" << x1 << ',' << y1 << '}' << std::endl;
		if (y1 <= 0) {
	...
	while (flag == 1) {
		...
		if (y1 >= 0) {
			...
			/*	if (y1 <= 0) {
					x1 = gun_1.x + speed_1.x * ((t-0.2) + d_t);
					y1 = gun_1.y + speed_1.y * ((t-0.2) + d_t) - g * pow(((t-0.2) + d_t), 2) * 0.5;
				} */
				//	std::cout << x1 << '\t' << y1 << '\t';
		}
		else {
		...
		if (y2 >= 0) {
			...
			/*	if (y2 <= 0) {
					x2 = gun_2.x + speed_2.x * (t - 0.2);
					y2 = gun_2.y + speed_2.y * (t - 0.2) - g * pow(((t - 0.2)), 2) * 0.5;
				} */
